...
class HEIC():
    ...
    def convert_heic_to_jpg(
        ...
        heic_files = HEIC.find_heic_files(heic_dir, recursive=recursive)
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_file = {
                executor.submit(HEIC.convert_single_file, heic_path, jpg_path, output_quality, resize): heic_path
        ...
        # Print summary
        global heic_output
        heic_output =  f"\nConversion completed in {total_time:.2f} seconds.\nSuccessfully converted: {num_converted}/{len(tasks)} files."
        print(f"{heic_output}")
        print(f"Successfully converted: {num_converted}/{len(tasks)} files.")
        if failed_files:
            heic_output =  f"Failed to convert {len(failed_files)} files."
    ...
    """
        ...
        result = HEIC.convert_heic_to_jpg(
            heic_dir, 
            args.quality, 
            args.workers,
            args.output,
        ...
        if result["status"] == "error":
            #heic_output = f"Error: {result['message']}"
            #print(f"{heic_output}")
            return f"Error: {result['message']}"
        elif result["status"] == "aborted":
            #heic_output = f"Aborted: {result['message']}"
            #print(f"{heic_output}")
            return f"Aborted: {result['message']}"
